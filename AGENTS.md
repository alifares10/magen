...
- After each meaningful task, run lint and relevant tests.
- Before ending a session or marking major work complete, run:
  - `lint`
  - `test`
  - `typecheck`
  - `build`
...
- Preserve strong type safety across the codebase.
- Do not use `any` unless absolutely unavoidable.
- Prefer explicit types, inferred types, and shared schema-derived types.
- If a type is unknown, model it properly or use `unknown` and narrow it safely.
